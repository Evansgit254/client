    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-100">
      <div className="bg-white p-8 rounded-lg shadow-lg max-w-md w-full">
        ...
        <form className='flex flex-col space-y-5' onSubmit={(e)=>handleSubmit(e)}>
          {
            error.msg ? <div className={`${error.status === "success" ? "bg-green-100 text-green-500" : "bg-red-100 text-red-500" } px-2 py-1 rounded-lg`}>
            <p>{error.msg}</p>
          </div> : <></>
          }
          <div className="mb-4">
            <label className="block text-sm font-bold mb-2" htmlFor="email">Email<span className="text-red-500">*</span></label>
            <input onChange={(e)=>changeBody(e)} name="email" value={body.email} className="w-full px-3 py-2 border rounded-lg" id="email" type="email" placeholder="Email"/>
          </div>
          <div className="mb-4">
            <label className="block text-sm font-bold mb-2" htmlFor="password">Password<span className="text-red-500">*</span></label>
            <input onChange={(e)=>changeBody(e)} name="password" value={body.password} className="w-full px-3 py-2 border rounded-lg" id="password" type="password" placeholder="Password" />
          </div>
          <button type='submit' className="w-full bg-purple-700 text-white py-2 px-4 rounded-lg">Log In</button>
        </form>
      </div>
    </div>
  );
